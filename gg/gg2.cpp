...
std::string SendAPIRequest(const std::string& input) {
    const std::string apiKey = "AIzaSyAGnJyn5nUVP81DZ-FIQMeNzY803KXjAr4";  // Replace with your API key
    const std::string apiUrl = "https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateText?key=" + apiKey;
    ...
    HINTERNET hRequest = HttpOpenRequest(hConnect, "POST", ("/v1/models/gemini-pro:generateText?key=" + apiKey).c_str(), NULL, NULL, NULL, INTERNET_FLAG_SECURE, 0);
    if (!hRequest) {
    ...
    std::string postData = "{\"prompt\": \"" + input + "\", \"max_tokens\": 100}";
